...
public:
  ...
  static const std::array<Direction, 6>
      edgeDirections; //!< An array of the 6 edge directions
  static const std::array<Direction, 6>
      vertexDirections; //!< An array of the 6 vertex directions
  static const std::array<Direction, 8>
      allDirections; //!< An array of all directions

  /// An array of 2 directions representing an alternative representation of an
  /// edge or vertex. The first Direction is the direction to travel to get to
  /// the tile the element is in. The second Direction is the direction of the
  /// target edge or vertex, once you're in the target cell For example, the
  /// Southeastern vertex in a cell, has an alternative representation of {EAST,
  /// SOUTHWEST}, or {SOUTHEAST, NORTHWEST}
  ...
  getComplementaryVertexRepresentations(Direction d);

  /**
   * @brief Get the locally adjacent edges of edge \p d (aka the two edges that
   * are available within the local tile)
   *
  ...
  /**
   * @brief Get all representations of the two edges of edge \p d that must be
   ...
   * with @ref getAdjacentEdgeToVertexDirections to get all neighbouring edges
   * to a vertex
   *
   * @param d
   ...
   * getAdjacentVertexToVertexDirections to get all neighbouring vertices to a
   * vertex
   *
   * @param d The direction of the target vertex
   * @return std::array<elementRepresentation, 2> The two representations of the
   * vertex that are "behind" the target vertex
   */
  static std::array<vertexRepresentation, 2> getSecondDegreeVertex(Direction d);
