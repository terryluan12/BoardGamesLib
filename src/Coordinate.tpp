...
namespace std {
    template<size_t N>
    struct tuple_size<Coordinate<N>>
    : integral_constant<size_t, N> {};
    
    template<size_t N, size_t M>
    struct tuple_element<M, Coordinate<N>> {
...
template<size_t N>
int Coordinate<N>::operator[](const size_t ci) const {
    if(ci < N) {
        return coordinates[ci];
    }
    throw std::out_of_range("Error: attempting to access Coordinate of size" + std::to_string(coordSize) + " with index " + std::to_string(ci));
}
...
template<size_t N>
int Coordinate<N>::getX(void) const {
    return coordinates[0];
}

template<size_t N>
int Coordinate<N>::getY(void) const {
