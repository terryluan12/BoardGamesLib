...
TEST_F(BankFixture, DefaultConstructor) {
  ...
  EXPECT_EQ(dBank.getResourceCount(), expected) << "Default resources should be 0";
  EXPECT_EQ(dBank.getDevelopmentCount(), expected) << "Default developments should be 0";
}

TEST_F(BankFixture, AddResource) {
  std::array<size_t, 5> expected = {5, 5, 5, 5, 5};
  dBank.addResources({5, 5, 5, 5, 5});
  EXPECT_EQ(dBank.getResourceCount(), expected) << "Resources should be added correctly";
...
TEST_F(BankFixture, RemoveResource) {
  ...
  EXPECT_EQ(pBank.getResourceCount(), expected) << "Resources should be removed correctly";
...
TEST_F(BankFixture, RemoveResourceEmpty) {
  EXPECT_THROW({
    pBank.removeResource(ResourceType::WOOD, 11);
    }, std::invalid_argument) << "Doesn't throw an error when removing 11 woods from " << pBank;
  EXPECT_THROW({
    dBank.removeResource(ResourceType::WOOD, 1);
    }, std::invalid_argument);
}

TEST_F(BankFixture, CanAfford) {
  EXPECT_TRUE(pBank.canAfford({10, 11, 12, 13, 14})) << "Should be able to afford";
  ...
  pBank.removeResources({5, 5, 5, 5, 5});
  EXPECT_FALSE(pBank.canAfford({6, 1, 2, 3, 4})) << "Should not be able to afford";
}

TEST_F(BankFixture, PopDevelopment) {
  DevelopmentType expected = DevelopmentType::VICPOINT;
  EXPECT_EQ(pBank.popDevelopment(), expected) << "Should pop the last development";
  ...
  EXPECT_EQ(pBank.getDevelopmentCount(), newExpected) << "Should have no developments left";
