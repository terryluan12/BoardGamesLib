...
DoganVertex::DoganVertex(std::initializer_list<std::variant<Direction, Coordinate<2>>> parameters) : isReady(false) {
    if(parameters.size() == 2) {
        auto it = parameters.begin();
        direction = std::get<Direction>(*it);
        coordinate = std::get<Coordinate<2>>(*(it + 1));
    }else {
...
void DoganVertex::ready(std::map<Coordinate<2>, std::shared_ptr<DoganCell>> cells) {
