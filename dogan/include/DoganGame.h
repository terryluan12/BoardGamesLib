...
public:
   ...
   * You only need to provide one representation of the location to build the structure. 
   * The function will automatically determine the other representations and will populate.
   * 
   * @param playerID The player ID of the player building the structure
   * @param structType The Structure Type to build
   * @param tileLocation A Coordinate2D object representing a tile to build the structure
   * @param direction The direction to build the structure
   * @param cost A cost array representing the cost of the structure. Should be positive integers
   ...
   * @param resources1 The resources the first player is trading to the second. If negative, indicates the player is receiving resources
   * @param playerID2 The player ID of the second player
   * @param resources2 The resources the second player is trading to the first. If negative, indicates the player is receiving resources
   */
   ...
   * @param tileLocation A Coordinate2D object representing the tile to place the robber
   * @param direction The direction of the structure to steal from. If the tile has no structures, use Direction::NONE
   */
  void useRobber(int playerID, Coordinate2D tileLocation, Direction direction);

  /**
   * @brief Use the monopoly development card.
   * The monopoly development card allows the player to take all of a certain resource from all other players
   * 
   ...
   * The soldier development card allows the player to move the robber and steal a resource from a player. It also
   * increases the soldier count, and if the player has either 5 soldiers played or the most soldiers played
   * (the higher of the two), they get an additional 2 victory points 
   * 
   ...
   * @param direction The direction to steal from. If the tile has no structures, use Direction::NONE
   */
   ...
   * @param tileLocations An array of two Coordinate2D objects representing the tiles to build the roads on
   * @param directions An array of two Direction enums representing the directions to build the roads
   */
   ...
   * 
   * @param playerID The player ID of the player to get the count from. If -1, get the bank resource count
   * @return const std::array<int, 5> An array of integers representing the resources of the player/bank
   */
  const std::array<int, 5> getResourceCount(int playerID) const;
  /**
   * @brief Gets an array representation of all the development cards a player has
   * 
   * @param playerID The player ID of the player to get the count from. If -1, get the bank development count
   * @return const std::array<int, 5> An array of integers representing the development cards of the player/bank
   */
...
private:
  ...
  std::uniform_int_distribution<pip> die; //!< A random number generator for dice rolls
  ...
  std::pair<int, int> mostSoldiers; //!< The player with the most soldiers played and how many soldiers they've played
  std::pair<int, int> longestRoad; //!< The player with the longest road and how many roads they have
  std::mt19937 rengine;
