...
public:
  ...
  DoganConfigBuilder &
  setTileLocations(std::vector<Coordinate2D> tileLocations);
  ...
  DoganConfigBuilder &
  setDevelopmentConfig(Configuration developmentConfig);
  DoganConfigBuilder &setNumberConfig(Configuration numberConfig);
  DoganConfigBuilder &
  setPortResourceConfig(Configuration portResourceConfig);
